...
if ($currentView === 'pages') {
    ...
    $contentVars = $viewModel !== null
        ? ['pages' => $viewModel->pages, 'searchQuery' => $viewModel->searchQuery, 'userName' => $viewModel->userName]
        : [];
} else {
    $title       = 'Dashboard - Haarlem CMS';
    $contentPath = __DIR__ . '/../../partials/cms/_dashboard-content.php';
    $contentVars = $viewModel !== null
        ? ['recentPages' => $viewModel->recentPages, 'activities' => $viewModel->activities, 'userName' => $viewModel->userName]
        : [];
}
