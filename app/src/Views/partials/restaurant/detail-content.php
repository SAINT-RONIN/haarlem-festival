 ...
 *
 * Expects a \App\ViewModels\Restaurant\RestaurantDetailViewModel as $viewModel.
 * Each page section reads from a small section ViewModel (like History does).
 *
...
// Extract section ViewModels (same pattern as History partials)
/** @var ContactSectionData $contact */
$contact = $viewModel->contactSection;
/** @var PracticalInfoSectionData $practical */
$practical = $viewModel->practicalInfoSection;
/** @var GallerySectionData $gallery */
$gallery = $viewModel->gallerySection;
/** @var AboutSectionData $about */
$about = $viewModel->aboutSection;
/** @var ChefSectionData $chef */
$chef = $viewModel->chefSection;
/** @var MenuSectionData $menu */
$menu = $viewModel->menuSection;
/** @var LocationSectionData $location */
$location = $viewModel->locationSection;
/** @var ReservationSectionData $reservation */
$reservation = $viewModel->reservationSection;

// Helper: escapes text for safe HTML output.
$e = function (string $text): string {
    return htmlspecialchars($text, ENT_QUOTES, 'UTF-8');
};
// Helper: escapes an image path for safe use in src="".
...
<section class="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-5">
    <div class="max-w-7xl mx-auto flex flex-col lg:flex-row items-stretch gap-8 lg:gap-12">
        ...
        <div class="flex-1 pb-6 bg-white rounded-2xl flex flex-col overflow-hidden">
            <div class="px-4 py-6 bg-royal-blue">
                <h2 class="text-white text-2xl sm:text-3xl font-medium font-['Montserrat']"><?= $e($practical->labelTitle) ?></h2>
            </div>
            ...
            <div class="px-5 py-4 flex items-start gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <span class="text-slate-800 text-lg font-bold font-['Montserrat']"><?= $e($practical->labelPriceFood) ?></span>
                    <?php foreach ($practical->priceCards as $pc): ?>
                        <span class="text-slate-800 text-lg font-normal font-['Montserrat']"><?= $e($pc['label']) ?>: <?= $e($pc['price']) ?></span>
                    <?php endforeach; ?>
                    <span class="text-slate-800 text-lg font-normal font-['Montserrat']"><?= $e($practical->labelCuisineType) ?> <?= $e($practical->cuisine) ?></span>
                </div>
            </div>
            ...
            <div class="px-5 py-4 flex items-start gap-4">
                ...
                <div class="flex flex-col gap-1">
                    <span class="text-slate-800 text-lg font-bold font-['Montserrat']"><?= $e($practical->labelRating) ?></span>
                    <span class="text-slate-800 text-lg font-normal font-['Montserrat']">
                        <?= $e($practical->labelFestivalRated) ?> <?= str_repeat('★', $practical->rating) ?>
                    </span>
                    <?php if ($practical->michelinStars > 0): ?>
                        <span class="text-slate-800 text-lg font-normal font-['Montserrat']">
                            <?= $practical->michelinStars ?> <?= $e($practical->labelMichelin) ?><?= $practical->michelinStars > 1 ? 's' : '' ?>
                        </span>
                    <?php endif; ?>
                </div>
            </div>
            ...
            <!-- Special Requests -->
            <?php if ($practical->specialRequestsNote !== ''): ?>
            <div class="px-5 py-4 flex items-start gap-4">
                <div class="w-12 h-12 flex-shrink-0 flex items-center justify-center">
                    <svg class="w-8 h-8 text-slate-800" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24" aria-hidden="true">
                ...
                <div class="flex flex-col gap-1">
                    <span class="text-slate-800 text-lg font-bold font-['Montserrat']"><?= $e($practical->labelSpecialRequests) ?></span>
                    <span class="text-slate-800 text-lg font-normal font-['Montserrat']"><?= $e($practical->specialRequestsNote) ?></span>
                </div>
            </div>
            <?php endif; ?>
        </div>
...
<section class="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-5">
    <div class="max-w-7xl mx-auto flex flex-col items-center gap-6">
        ...
        <div class="w-full flex flex-col md:flex-row justify-center items-center gap-8 lg:gap-12">
            <img class="flex-1 min-w-0 h-64 sm:h-80 md:h-96 lg:h-[450px] rounded-2xl shadow-lg object-cover" src="<?= $img($gallery->image1) ?>" alt="<?= $e($viewModel->name) ?> gallery photo 1"/>
            <img class="flex-1 min-w-0 h-64 sm:h-80 md:h-96 lg:h-[450px] rounded-2xl shadow-lg object-cover" src="<?= $img($gallery->image2) ?>" alt="<?= $e($viewModel->name) ?> gallery photo 2"/>
            <img class="flex-1 min-w-0 h-64 sm:h-80 md:h-96 lg:h-[450px] rounded-2xl shadow-lg object-cover" src="<?= $img($gallery->image3) ?>" alt="<?= $e($viewModel->name) ?> gallery photo 3"/>
        </div>
    </div>
</section>
...
<!-- ============================================= -->
<?php if ($menu->description !== '' || !empty($menu->cuisineTags)): ?>
<section class="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-8 sm:py-12">
    <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12 items-center">
        ...
        <div class="flex-1 flex items-center justify-center gap-4 sm:gap-6 max-w-[45%]">
            <img class="flex-1 min-w-0 h-48 sm:h-56 lg:h-64 rounded-2xl object-cover" src="<?= $img($menu->image1) ?>" alt="Menu dish 1"/>
            <img class="flex-1 min-w-0 h-48 sm:h-56 lg:h-64 rounded-2xl object-cover" src="<?= $img($menu->image2) ?>" alt="Menu dish 2"/>
        </div>
    </div>
</section>
<?php endif; ?>
...
<section id="reservation" class="px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 py-8 sm:py-12">
    <div class="max-w-7xl mx-auto flex flex-col lg:flex-row gap-8 lg:gap-12 items-center">
        ...
        <div class="flex-1 flex flex-col gap-8">
            <h2 class="text-slate-800 text-4xl sm:text-5xl lg:text-6xl font-bold font-['Montserrat']"><?= $e($reservation->labelTitle) ?></h2>
            <p class="text-slate-800 text-lg sm:text-xl font-normal font-['Montserrat'] leading-7">
                <?= $e($reservation->labelDesc) ?>
            </p>

            <!-- Price & Info Cards -->
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                <?php foreach ($reservation->priceCards as $pc): ?>
                ...
                <div class="px-4 py-5 bg-white rounded-lg flex flex-col items-center gap-3">
                    ...
                    <span class="text-slate-800 text-lg font-medium font-['Montserrat']"><?= $e($reservation->labelDuration) ?></span>
                ...
                <div class="px-4 py-5 bg-white rounded-lg flex flex-col items-center gap-3">
                    ...
                    <span class="text-slate-800 text-lg font-medium font-['Montserrat']"><?= $e($reservation->labelSeats) ?></span>
            ...
            <?php if (!empty($reservation->timeSlots)): ?>
            <div class="flex flex-col gap-4">
                <h3 class="text-slate-800 text-lg font-medium font-['Montserrat']"><?= $e($reservation->labelSlots) ?></h3>
        ...
        <!-- Reservation Image -->
        <div class="flex-1 flex items-center justify-center">
            <img class="w-full h-auto max-h-[700px] rounded-2xl object-cover" src="<?= $img($reservation->image) ?>" alt="<?= $e($viewModel->name) ?> reservation"/>
        </div>
    </div>
</section>
