<?php
/**
 * Storytelling schedule section partial.
 *
 * @var \App\ViewModels\Schedule\StorytellingScheduleViewModel $scheduleSection
 */
...
<!-- Schedule Section -->
<section id="storytelling-schedule"
         class="w-full flex flex-col justify-start items-center"
         aria-labelledby="storytelling-schedule-heading">
    ...
    <div class="w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 pb-3.5 flex flex-col justify-start items-start gap-3.5 overflow-hidden">
        ...
        <header class="w-full flex flex-col justify-start items-start gap-3.5">
            <div class="w-full flex flex-col sm:flex-row justify-start items-start sm:items-end gap-2.5 overflow-hidden">
                <h2 id="storytelling-schedule-heading"
                    class="flex-1 text-slate-800 text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-bold font-['Montserrat']">
                    <?= CmsOutputHelper::text($schedule->title) ?>
                </h2>
                <span class="text-slate-800 text-2xl sm:text-3xl md:text-4xl font-bold font-['Montserrat']">
                    <?= CmsOutputHelper::text($schedule->year) ?>
                </span>
            </div>
            <div class="w-full h-px bg-[#1A2A40]" aria-hidden="true"></div>
        </header>
        ...
        <?php if ($schedule->showAdditionalInfo && !empty($schedule->additionalInfoBody)): ?>
            <aside class="w-full p-3.5 bg-blue-100 rounded-2xl border-3 border-[#1C398E] flex flex-col justify-center items-start gap-2.5">
    ...
    <!-- Story Count -->
    <?php if ($schedule->showStoryCount): ?>
        <div class="w-full px-4 sm:px-8 md:px-12 lg:px-16 xl:px-24 flex justify-center items-center gap-2.5">
            <p class="flex-1 text-right text-slate-800 text-base sm:text-lg font-medium font-['Montserrat'] leading-7"
               aria-live="polite">
                <?= (int)$schedule->storyCount ?> <?= CmsOutputHelper::text($schedule->storyCountLabel) ?>
            </p>
        </div>
    <?php endif; ?>
